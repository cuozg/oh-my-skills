...
description: >
  ...
  CLI (scripts/image_gen.py) and requires GEMINI_API_KEY or GOOGLE_API_KEY.
---

# Image Generation Skill

Generates or edits images for the current project using Google Gemini's Imagen API. Defaults to `imagen-4.0-generate-001` and the bundled CLI (`scripts/image_gen.py`) for deterministic, reproducible runs.
...
## Environment
- **`GEMINI_API_KEY`** (or `GOOGLE_API_KEY`) required for live API calls (no API key = dry-run only).
- If missing: user creates key at https://aistudio.google.com/apikey and sets it locally.
