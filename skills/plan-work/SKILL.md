---
name: plan-work
description: "Pick one incomplete goal, implement it end-to-end, and verify no errors after implementation. Autonomous single-goal execution pipeline for Docs/Goals goal files. Runs goal through fixed specialist phases: discover goal, plan, review, implement on a goal branch in the repo root by default, optionally use a worktree only when requested, independently verify every acceptance criterion with concrete evidence, then sync goal docs, Master.md, README, and specs. Use when the user says 'execute goal', 'run goal', 'plan work', 'do the goal', 'start working', 'execute the plan', 'implement everything', 'autonomous mode', 'just do everything', or `/omo/work`. Do NOT use for creating goals; use plan-goal. Do NOT use for verification-only reports; use plan-test."
---
...
4. **Git enforces isolation.** By default, Sisyphus works directly in the repo root on a `goal/<slug>` branch — isolation is preserved via branch, not directory. Use a dedicated worktree outside the repo root only when **worktree mode** is explicitly active (user passes `--worktree`, says "use worktree", or local policy requires directory isolation).
...
| 8 | **Default to no-worktree mode.** Work directly in the repo root on a `goal/<slug>` branch unless the user explicitly requests worktree mode. |
| 9 | **Always use branch `goal/<slug>`**, whether in no-worktree mode or worktree mode. In no-worktree mode, `git switch -c goal/<slug>` in the repo root replaces `git worktree add`. |
...
| 12 | **Always clean up the execution context after PR creation.** In no-worktree mode, switch back to the base branch. In worktree mode, remove the worktree. On PR creation failure after push, preserve the remote branch for manual recovery. |
...
| 1 | orchestrator | Read goal + spec, detect domain, create branch (or optional worktree), create tracking tasks | n/a | n/a |
...
| 4 | `sisyphus-junior` | Implement in repo root by default (or optional worktree mode), three-gate verify, commit, push, `gh pr create` | Yes | `sisyphus_session_id` reused on Hephaestus FAIL |
...
5. **Create the branch / optional worktree** — default is no-worktree mode:

**Default (no-worktree mode):**
```bash
git fetch origin
git switch -c goal/<slug> origin/<base-branch>  # from repo root
```
- Slug = kebab-case of the goal title with `[Feature]` stripped (`[Combat] Add parry` → `combat-add-parry`).
- Set `worktree_path = <repo_root>` (the repo itself).
- Set `no_worktree_mode = true` in the orchestrator scratchpad.
- Skip all `worktree_manager.sh` calls — the branch is the isolation unit.
- Record: `worktree_path=<repo_root>`, `branch=goal/<slug>`, `base_branch`.

**Worktree mode** (only when user said "use worktree", passed `--worktree`, or local policy requires directory isolation): Use the bundled helper:
```
run_skill_script(
  skill="plan-work",
...
```
- Helper creates branch `goal/<slug>` and worktree at `../.worktrees/<slug>`.
- Set `no_worktree_mode = false`.
...
- Prompt carries: full goal (objective, context, criteria verbatim numbered 1..N, constraints), spec content (or "no spec"), working env (`worktree_path` + `no_worktree_mode`), domain.
...
Sisyphus executes the approved plan, runs three-gate verification, and creates the PR. Use the template in `references/delegation-templates.md` §4.
...
- Prompt carries: working env (`worktree_path`, `no_worktree_mode`), full goal, spec, **approved plan verbatim**, three-gate protocol, commit+push+PR instructions, self-review checklist.
- In **no-worktree mode** (default), the Sisyphus prompt omits the `Do NOT modify files outside the worktree` restriction (it IS the repo root); all `git -C <worktree>` commands become plain `git` commands run in the repo root.
- In **worktree mode**, the Sisyphus prompt enforces worktree-only file modifications.
...
2. `git -C <worktree_path> push -u origin <branch>` (in no-worktree mode, omit `-C` or use repo root path).
...
No-worktree mode is the default. Branch management is handled with plain `git switch / git push` in the repo root. There are no worktree directories to create, remove, or prune.

The bundled `scripts/worktree_manager.sh` is the canonical path for all worktree lifecycle operations (Optimization O1). **Use it only when worktree mode is explicitly active.**

### Create (Step 1 — worktree mode only)
```
...
### Cleanup on success (worktree mode only)
...
**Default no-worktree cleanup:** After PR creation, simply switch back to the base branch:
```bash
git switch <base_branch>
```
Do NOT delete the `goal/<slug>` branch — it stays until the PR merges.

### Cleanup on failure (G3 fix)

**Worktree mode:** If `gh pr create` failed **but the branch was pushed**, the worktree may be orphaned. Take the fallback path:
...
**No-worktree mode:** If `gh pr create` failed but the branch was pushed, switch back to base branch and log `PR_CREATION_FAILED: branch goal/<slug> pushed, create PR manually`. No directory cleanup needed.

### Other helper commands (worktree mode only)
...
| `t_branch_ready` | After Step 1 worktree created (worktree mode) or branch switched (no-worktree mode) |
...
| `t_branch_cleaned` | After worktree removal (worktree mode) or `git switch` back to base (no-worktree mode) |
...
2. **Check for progress** — if the stuck specialist was Sisyphus, run `run_skill_script('plan-work', 'scripts/worktree_manager.sh', arguments=['status', '<slug>'])` (worktree mode), or `git status` + `git log --oneline -5` in the repo root (no-worktree mode). Commits or a dirty tree mean real progress was made.
...
### Branch / Worktree
- Mode:   no-worktree | worktree
- Path:   {worktree_path}  (repo root in no-worktree mode)
...
| Crash recovery finds existing `goal/<slug>` with open PR | Resume on that branch in the repo root by default, or re-attach/create the worktree only if worktree mode is active |
...
| PR creation failed, branch pushed | Cleanup-on-failure path: remove worktree dir (worktree mode) or switch to base (no-worktree mode); preserve remote branch, log orphan |
...
- Modifying files while on the wrong branch in no-worktree mode.
- Modifying files outside the worktree when worktree mode is active.
...
- **`scripts/worktree_manager.sh`** — the canonical helper for create / remove / list / cleanup / status when worktree mode is explicitly active. Prefer it over inline `git worktree` commands in that mode. (Optimization O1.)
