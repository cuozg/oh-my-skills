#!/usr/bin/env python3
"""
Check test coverage for refactored Unity C# code.

Analyzes refactored source files and their corresponding test files to identify
coverage gaps. Does NOT require running Unity — performs static analysis only.

Checks:
1. Every refactored class has a corresponding test file
2. Public methods have test coverage (by name matching)
3. Test-to-source ratio is reasonable
4. Critical patterns (event handlers, state transitions) have tests

Usage:
    python3 check_test_coverage.py <source-dir> <test-dir>
    python3 check_test_coverage.py Assets/Scripts/Feature Assets/Scripts/Test/EditMode
    python3 check_test_coverage.py --source Assets/Scripts/MyClass.cs --test-dir Assets/Scripts/Test/EditMode
"""
...
class CoverageChecker:
    def __init__(self):
        self.gaps = []  # Missing coverage
        self.covered = []  # Confirmed coverage
        self.warnings = []  # Potential issues
        self.stats = {
            "source_classes": 0,
            "source_methods": 0,
            "test_classes": 0,
    ...
    def extract_public_methods(self, filepath: str) -> List[Tuple[str, str, int]]:
        """Extract public method names from a C# file.
        Returns list of (class_name, method_name, line_number).
        """
        ...
        for i, line in enumerate(lines, 1):
            ...
            if mm and current_class:
                ...
                # Skip Unity lifecycle and property accessors
    ...
    def find_test_files(self, test_dir: str, class_name: str) -> List[str]:
        """Find test files that likely test a given class."""
        ...
        patterns = [
            ...
            f"{class_name}EventTests.cs",
            f"{class_name}IntegrationTests.cs",
    ...
    def check_method_coverage(
        ...
        """Check if a method has test coverage by name matching."""
        # Test name patterns that indicate coverage
        for test_name in test_names:
            # Direct match: ClassName_MethodName_...
            if method_name in test_name:
                self.covered.append(
                    f"{class_name}.{method_name} → covered by {test_name}"
                )
                self.stats["covered_methods"] += 1
                return True

        self.gaps.append(
            f"{source_file}:{line_num}: {class_name}.{method_name}() — no test found"
        )
        self.stats["uncovered_methods"] += 1
        return False

    def check_critical_patterns(self, filepath: str) -> None:
        """Check if critical patterns have corresponding tests."""
        content = Path(filepath).read_text(encoding="utf-8", errors="replace")

        critical = [
            (
                r"event\s+\w+\s+(\w+)",
                "Event '{}' should have subscription/firing tests",
            ),
    ...
    def analyze(self, source_paths: List[str], test_dir: str) -> None:
        ...
        for source_path in source_paths:
            ...
            for cs_file in cs_files:
                # Skip test files in source scan
    ...
    def report(self) -> int:
        """Print coverage report and return exit code."""
        total_methods = self.stats["covered_methods"] + self.stats["uncovered_methods"]
        coverage_pct = (
            (self.stats["covered_methods"] / total_methods * 100)
            if total_methods > 0
            else 0
        )
        ...
        print(
            ...
            f"({self.stats['covered_methods']}/{total_methods} methods)"
        )

        if self.covered:
            print(f"\n✅ COVERED ({len(self.covered)}):")
            for c in self.covered[:20]:  # Limit output
                print(f"   {c}")
