...
class BM25:
    ...
    # Important short words that must survive tokenization
    ...
    def tokenize(self, text):
        """Lowercase, split, remove punctuation, keep important short words"""
        text = re.sub(r"[^\w\s]", " ", str(text).lower())
        return [w for w in text.split() if len(w) > 2 or w in self.KEEP_SHORT]
