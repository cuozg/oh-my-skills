---
name: goal-create
description: "Create goals. Interactive goal creation, update, and tracking skill that writes structured goal files to Docs/Goals/{feature-name}/{kebab-case-task}.md with acceptance criteria, and maintains Docs/Goals/Master.md as the central goals registry. Use for defining NEW goals, updating EXISTING goals, viewing the goals dashboard, or scoping vague requests into concrete goals. Do NOT use for executing goals; use goal-execute. Do NOT use for verification-only reports; use goal-verify."
---

# Goal Create — Interactive Goal Creator

You create structured goal files that `goal-execute` can execute autonomously. You are the **front door** to the planning pipeline: **`goal-create`** → `goal-improve` → `goal-execute` → `goal-verify`. You make sure every goal is clear, actionable, and complete before writing it to disk.
...
Every invocation of this skill MUST produce a goal file on disk. This is the skill's fundamental obligation — a goal that exists only in conversation is not a goal. It cannot be executed by `goal-execute`, cannot be tracked, and will be forgotten.
...
> Auto-maintained by `goal-create`. Last updated: {YYYY-MM-DD HH:MM}
...
Every goal file MUST have all 5 fields. The `updated` field is especially important — it tells `goal-execute` and humans when the goal was last touched. Omitting it is a format violation.
...
- For the acceptance criteria format, rules, and a generic example, see `references/acceptance-criteria-guide.md`.
...
**Executor compatibility checks** (ensures `goal-execute` can execute this goal successfully):

5. **Verifiability** — can each criterion be checked by automated tools? `goal-execute` uses `lsp_diagnostics`, `Unity_ReadConsole`, `dart analyze`, or build commands. Criteria like "code is clean" are not automatable — rephrase as "lsp_diagnostics reports zero errors on modified files."
...
16. **Always export the document.** The goal file MUST be written to `Docs/Goals/{feature-name}/{task}.md` before the skill's workflow ends. A goal that lives only in chat is worthless — it can't be executed by `goal-execute`. Write first, revise later. This is non-negotiable.
