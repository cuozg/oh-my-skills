...
### Definition of Done
- [x] `ls -d skills/*-shared skills/git-shared skills/bash-shared | wc -l` → 9 (all shared skills exist)
- [x] `find skills/ -name "post_review.py" | wc -l` → 1
- [x] `find skills/ -name "trace_logic.py" | wc -l` → 1
- [x] `test ! -d skills/unity-code-standards && echo "RENAMED"` → RENAMED
- [x] `wc -l skills/*/SKILL.md | awk '$1 >= 100 {print}' | grep -v skill-creator | wc -l` → 0
- [x] `grep -r 'unity-code-standards' skills/ README.md --include='*.md' | wc -l` → 0
- [x] `git diff skills/skill-creator/ | wc -l` → 0
- [x] `python3 -m pytest skills/unity-review-shared/scripts/tests/ skills/unity-investigate-shared/scripts/tests/ skills/unity-plan-shared/scripts/tests/` → all pass
- [x] `skill-validator` passes for all 42 skills (34 original + 8 new shared)
...
- [x] 1. Baseline Validation Sweep
  ...
  **Acceptance Criteria**:
  - [x] Baseline report saved with MD5 checksums verified identical
...
- [x] 2. Create `unity-review-shared/` + Move `post_review.py`
  ...
  **Acceptance Criteria**:
  - [x] `find skills/ -name 'post_review.py' | wc -l` → 1
  - [x] `python3 -m pytest skills/unity-review-shared/scripts/tests/test_post_review.py` → PASS
  - [x] `skill-validator` passes for unity-review-shared
  - [x] No empty `scripts/` dirs in review skills
...
- [x] 3. Create `unity-investigate-shared/` + Move `trace_logic.py`
  ...
  **Acceptance Criteria**:
  - [x] `find skills/ -name 'trace_logic.py' | wc -l` → 1
  - [x] `python3 -m pytest skills/unity-investigate-shared/scripts/tests/test_trace_logic.py` → PASS
  - [x] `skill-validator` passes for unity-investigate-shared
...
- [x] 4. Create Minimal Shared Scaffolds: git-shared, bash-shared, unity-test-shared, unity-document-shared
  ...
  **Acceptance Criteria**:
  - [x] All 4 exist with valid frontmatter; `skill-validator` passes; all <30L; each has activation guard
...
- [x] 5. Create `unity-debug-shared/` + Move `common-fixes.md`
  ...
  **Acceptance Criteria**:
  - [x] `unity-debug-shared/references/common-fixes.md` exists; original location does NOT
  - [x] `unity-debug-quick/SKILL.md` references `../unity-debug-shared/`
  - [x] `skill-validator` passes for both
...
- [x] 6. Rename `unity-code-standards/` → `unity-code-shared/` + Update ALL References
  ...
  **Acceptance Criteria**:
  - [x] `test -d skills/unity-code-shared && test ! -d skills/unity-code-standards`
  - [x] `grep -r 'unity-code-standards' skills/ README.md --include='*.md' | wc -l` → 0
  - [x] `skill-validator` passes for unity-code-shared
  - [x] `skill-deps` shows no broken references from consumer skills
...
- [x] 7. Update 4 Review Skills' SKILL.md for Shared Script Paths
  ...
  **Acceptance Criteria**:
  - [x] All 5 review SKILL.md files reference `unity-review-shared` for post_review.py
  - [x] `grep -rn 'scripts/post_review' skills/unity-review-*/SKILL.md` shows only `unity-review-shared/` paths
  - [x] All 5 pass `skill-validator`
...
- [x] 8. Update 2 Investigate Skills' SKILL.md for Shared Script Paths
  ...
  **Acceptance Criteria**:
  - [x] Both SKILL.md files reference `../unity-investigate-shared/scripts/trace_logic.py`
  - [x] No investigate skill has a local `scripts/trace_logic.py`
  - [x] Both pass `skill-validator`
...
- [x] 9. Move code-deep's 3 Shared Refs to `unity-code-shared/` + Update code-quick
  ...
  **Acceptance Criteria**:
  - [x] 3 files exist in `unity-code-shared/references/`; do NOT exist in `unity-code-deep/references/`
  - [x] `grep 'unity-code-deep/references' skills/unity-code-quick/SKILL.md` → 0 matches
  - [x] `grep 'unity-code-shared/references' skills/unity-code-quick/SKILL.md | wc -l` → ≥3
  - [x] `skill-validator` passes for code-shared, code-deep, code-quick
...
- [x] 10. Extract Workflow: bash-check, bash-install, bash-optimize
  ...
  **Acceptance Criteria**:
  - [x] Each has `references/workflow.md`; all SKILL.md <100L; all refs <100L; `skill-validator` passes for all 3

  **Commit**: YES — `refactor(skills): extract workflow refs for bash-check, bash-install, bash-optimize`

- [x] 11. Extract Workflow: git-comment, git-commit
  ...
  **Acceptance Criteria**:
  - [x] Each has `references/workflow.md`; git-comment reduced to <70L; `skill-validator` passes

  **Commit**: YES — `refactor(skills): extract workflow refs for git-comment, git-commit`

- [x] 12. Extract Workflow: git-description, git-squash
  ...
  **Acceptance Criteria**:
  - [x] Each has `references/workflow.md`; `skill-validator` passes

  **Commit**: YES — `refactor(skills): extract workflow refs for git-description, git-squash`

- [x] 13. Extract Workflow: flatbuffers-coder
  ...
  **Acceptance Criteria**:
  - [x] Has `references/workflow.md`; SKILL.md <100L; `skill-validator` passes

  **Commit**: YES — `refactor(skills): extract workflow refs for flatbuffers-coder`

- [x] 14. Extract Workflow: unity-code-deep, unity-code-editor
...
- [x] 15. Extract Workflow: unity-code-quick
...
- [x] 16. Extract Workflow: unity-debug-deep, unity-debug-log
...
- [x] 17. Extract Workflow: unity-debug-quick
  ...
  **Acceptance Criteria**:
  - [x] Has `references/workflow.md` and `references/tool-selection.md`; SKILL.md reduced; `skill-validator` passes

  **Commit**: YES — `refactor(skills): extract workflow refs for unity-debug-quick`

- [x] 18. Extract Workflow: unity-investigate-quick, unity-investigate-deep
...
- [x] 19. Extract Workflow: unity-review-code-local, unity-review-code-pr
  ...
  **Acceptance Criteria**:
  - [x] Both have `references/workflow.md`; unity-review-code-pr reduced to <80L; `skill-validator` passes

  **Commit**: YES — `refactor(skills): extract workflow refs for unity-review-code-local, unity-review-code-pr`

- [x] 20. Extract Workflow: unity-review-architecture, unity-review-asset
  ...
  **Acceptance Criteria**:
  - [x] Both have `references/workflow.md`; both reduced to <80L; `skill-validator` passes

  **Commit**: YES — `refactor(skills): extract workflow refs for unity-review-architecture, unity-review-asset`

- [x] 21. Extract Workflow: unity-review-general, unity-review-prefab
...
- [x] 22. Extract Workflow: unity-review-quality
  ...
  **Acceptance Criteria**:
  - [x] Has `references/workflow.md`; reduced to <65L; `skill-validator` passes

  **Commit**: YES — `refactor(skills): extract workflow refs for unity-review-quality`

- [x] 23. Extract Workflow: unity-document-system, unity-document-tdd
  ...
  **Acceptance Criteria**:
  - [x] Both have `references/workflow.md`; `skill-validator` passes

  **Commit**: YES — `refactor(skills): extract workflow refs for unity-document-system, unity-document-tdd`

- [x] 24. Extract Workflow: unity-test-unit, unity-test-case
...
- [x] 25. Extract Workflow: mermaid
  ...
  **Acceptance Criteria**:
  - [x] Has `references/workflow.md`; `skill-validator` passes

  **Commit**: YES — `refactor(skills): extract workflow refs for mermaid`

### Wave 6 — Tracing Unification + Dedup Audit

- [x] 26. Create Unified Tracing Module in `unity-investigate-shared/`
  ...
  **Acceptance Criteria**:
  - [x] `trace_unified.py` exists with 3 modes
  - [x] Each mode produces equivalent output to original script
  - [x] `python3 -m pytest skills/unity-investigate-shared/scripts/tests/test_trace_unified.py` → PASS
...
- [x] 27. Update Document Skills to Use Unified Tracing
  ...
  **Acceptance Criteria**:
  - [x] `test ! -f skills/unity-document-system/scripts/trace_system.py`
  - [x] `test ! -f skills/unity-document-tdd/scripts/trace_architecture.py`
  - [x] Both SKILL.md files reference `trace_unified.py`; `skill-validator` passes

  **Commit**: YES — `refactor(skills): migrate document skills to unified tracing module`

- [x] 28. Audit All SKILL.md for SKILL.md ↔ Reference Content Duplication
  ...
  **Acceptance Criteria**:
  - [x] No >3 consecutive identical lines between any SKILL.md and its reference files
  - [x] All modified SKILL.md <100L; all modified refs <100L; `skill-validator` passes

  **Commit**: YES — `refactor(skills): remove SKILL.md \u2194 reference content duplication`

### Wave 7 — Validation

- [x] 29. Full Validation Sweep — All 42 Skills
  ...
  **Acceptance Criteria**:
  - [x] ALL skills pass `skill-validator`
  - [x] ALL SKILL.md <100L (except skill-creator)
  - [x] ALL reference files <100L
  - [x] ALL consumer skills have `references/workflow.md`
  - [x] Zero dangling references; zero duplicate scripts; zero `unity-code-standards` mentions
  - [x] skill-creator untouched; plan-* skills untouched
  - [x] All pytest suites pass
...
- [x] F3. **Reference Integrity QA** — `unspecified-high`
...
### Final Checklist
- [x] All "Must Have" present (9 shared skills, 29 workflow.md files, zero duplicates, rename complete)
- [x] All "Must NOT Have" absent (no skill-creator changes, no plan-* changes, no content rewrites, no requirement.txt dedup)
- [x] All tests pass
- [x] All 42 skills pass skill-validator
- [x] All cross-references resolve (skill-deps clean)
